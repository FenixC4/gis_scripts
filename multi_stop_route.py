...
if stops_layer.fields().indexFromName(order_field_name) == -1:
    raise Exception(f"Order field '{order_field_name}' not found in layer '{stops_layer_name}'")
...
try:
    ...
    print(f"Could not sort features. Ensure the '{order_field_name}' field contains sortable values (like numbers).")
    raise e
...
if len(sorted_stops_coords) < 2:
    raise Exception("Fewer than two stops found in the layer. Cannot calculate a route.")

print(f"Found and sorted {len(sorted_stops_coords)} stops from layer '{stops_layer_name}'.")
...
for i in range(len(sorted_stops_coords) - 1):
    ...
    params = {
        'INPUT': network_layer,
        'STRATEGY': 0,  # 0 for Shortest, 1 for Fastest
...
if route_segments:
    ...
    print("Multi-stop route created and added to the project as 'Multi-Stop Route (from Layer)'.")
else:
